...
namespace pavo
{
	...
	public:
		pavo_driver();
		pavo_driver(std::string dest_ip, uint16_t dest_port);

		pavo_driver(std::string device_ip, uint16_t device_port, std::string dest_ip, uint16_t dest_port);

		~pavo_driver();

		bool pavo_open(std::string device_ip, uint16_t device_port);
		void pavo_close();
